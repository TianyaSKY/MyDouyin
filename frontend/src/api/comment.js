...
export async function getVideoComments(token, videoId, current = 1, size = 20) {
    const resp = await apiFetch(`/api/comments/video/${videoId}?current=${current}&size=${size}`, token);
...
export async function getCommentReplies(token, commentId, current = 1, size = 20) {
    const resp = await apiFetch(`/api/comments/${commentId}/replies?current=${current}&size=${size}`, token);
    const json = await resp.json();
    if (json.code !== 200) {
