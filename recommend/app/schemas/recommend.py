...
class BatchVideoEmbeddingRequest(BaseModel):
    ...
    videos: Optional[List[BatchVideoEmbeddingItem]] = Field(
        None, description="批量视频信息"
    )
...
class VideoEmbeddingQueryRequest(BaseModel):
    """查询已存储视频向量请求"""
...
class InsertVideoEmbeddingRequest(BaseModel):
    ...
    embedding: List[float] = Field(
        ..., min_length=1024, max_length=1024, description="视频向量 (1024维)"
    )
...
class UpdateUserLongTermVectorRequest(BaseModel):
    ...
    vector: List[float] = Field(
        ..., min_length=1024, max_length=1024, description="长期向量 (1024维)"
    )
...
class InsertUserVectorRequest(BaseModel):
    ...
    user_id: int = Field(..., description="用户ID")
    long_term_vec: List[float] = Field(
        ..., min_length=1024, max_length=1024, description="长期向量 (1024维)"
    )
    interest_vec: List[float] = Field(
        ..., min_length=1024, max_length=1024, description="兴趣向量 (1024维)"
    )
...
class VectorRecallRequest(BaseModel):
    ...
    user_vector: List[float] = Field(
        ..., min_length=1024, max_length=1024, description="用户向量 (1024维)"
    )
